...
class Game:
    def __init__(self):
        #setting windoow configuration
        self.width=600
        self.height=768
        self.scale_factor=2.5
        self.scale_factor1=1.5
        self.win=pg.display.set_mode((self.width,self.height))
        self.bg_img=pg.transform.scale_by(pg.image.load("assets/bg.jpg").convert(),self.scale_factor)
        
        self.clock=pg.time.Clock()
        self.move_speed=250
        self.bird=Bird(self.scale_factor1)
    ...
    def checkCollisions(self):
        if len(self.pipes):
            if (self.bird.rect.bottom>568 or self.bird.rect.colliderect(self.pipes[0].rect_down) or self.bird.rect.colliderect(self.pipes[0].rect_up)):
    ...
    def updateEverything(self,dt):
          if self.is_enter_pressed:
            ...
            if self.pipe_generate_counter>70:
                self.pipes.append(Pipe(self.scale_factor1,self.move_speed))
    ...
    def drawEverything(self):
          self.win.blit(self.bg_img,(0,-100))
    ...
    def setUpBgAndGround(self):
        #loading images for bacgrround and ground
